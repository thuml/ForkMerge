...
class NYUv2(Dataset):
    ...
    def __init__(self, root, mode='train', augmentation=False):
        ...
        with open(os.path.join(current_path, 'data_split.json'), 'r') as f:
...
class Cityscapes(Dataset):
    ...
    def __init__(self, root, mode='train', augmentation=False):
        ...
        # calculate data length
        self.noise = torch.rand(len(self.index_list), 1, 256, 512)
        self.num_out_channels = {'segmentation': 19, 'depth': 1, 'normal': 3, 'noise': 1}
        self.num_classes = 19
    ...
    def __getitem__(self, i):
        ...
        image = torch.from_numpy(np.moveaxis(np.load(self.data_path + '/image/{:d}.npy'.format(index)), -1, 0))
        semantic = torch.from_numpy(np.load(self.data_path + '/label/{:d}.npy'.format(index)))
        semantic = F.upsample(semantic.unsqueeze(0).unsqueeze(0), scale_factor=2, mode='nearest').squeeze(0).squeeze(0)
        depth = torch.from_numpy(np.moveaxis(np.load(self.data_path + '/depth/{:d}.npy'.format(index)), -1, 0))
        noise = self.noise[i]
